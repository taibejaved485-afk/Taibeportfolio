  ...
  return (
    <section id="projects" className="pt-12 pb-32 relative">
      <div className="container mx-auto px-6">
        <ScrollReveal className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-20 md:mb-32 gap-10">
        ...
        <div className="mobile-slider lg:grid lg:grid-cols-3 lg:grid-rows-auto gap-6 pb-12">
          {projects.map((project, index) => (
            <ScrollReveal 
              key={index}
              delay={index * 100}
              className={`${project.gridClass} w-[85vw] lg:w-full group`}
            >
              <a 
                href={project.link} 
                target="_blank" 
                rel="noopener noreferrer"
                className="relative cursor-pointer block h-full rounded-[2.5rem] md:rounded-[3rem]"
              >
                {/* Orb Border Wrapper */}
